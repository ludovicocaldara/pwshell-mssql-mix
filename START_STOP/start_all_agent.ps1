...
foreach($strComputer in $compArray) {
	# Get-WMIObject Win32_Service  | Where-Object{$_.name -match ".*sqlagent\$.*" }
	...
	Get-WMIObject Win32_Service  -Computername $strComputer | Where-Object{$_.name -match ".*sqlagent\$.*" } | Sort-Object -Property Name | Format-Table Name, StartName, startmode, state, status
	$sqlservices = get-service -computername $strComputer -name SQLAgent$*
